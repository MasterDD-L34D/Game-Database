...
test('buildFuzzySearchSql parameterizes q/limit, uses % operator, quotes idents', () => {
  const sql = buildFuzzySearchSql({ entities: 'trait', q: 'lynks', limit: 20 });
  ...
  assert.match(sql.sql, /similarity/); // ranking score in SELECT
  assert.match(sql.sql, /%/); // GIN-accelerated match operator in WHERE
});

test('buildFuzzySearchSql Record arm emits NULL slug, no quoted slug column', () => {
  const sql = buildFuzzySearchSql({ entities: 'record', q: 'x', limit: 5 });
...
test('buildFuzzySearchSql includes one arm per selected entity', () => {
  const two = buildFuzzySearchSql({ entities: 'trait,biome', q: 'x', limit: 5 });
