...
- **Mid**: export species/biome/eco -- 🔴 **AGGIORNATO 2026-06-19 (RFC#4 S2 CHIUSO, REVERSAL)**. Species REAPERTO + TENTATO (S2) poi RESCOPATO a **fidelity-shadow ONLY** (#225 `abf8b04` / #224 `fa0deae` / #226 `0392458`): il DB MISURA il loss vs i file Game (shadow + fidelity report), NON scrive un export-on-release per-file. **biome/eco = CHIUSO import-only indefinitamente** (#227 `bc40887`). SoT authoring species RESTA file-side Game `data/core/species/species_catalog.json` (ADR-2026-05-15) -- il DB NON e' diventato upstream. Reopening reale = il **S3 DB-as-SoT co-design** (#228 `7d39845` SCOPING-ONLY, 7 domande aperte, raccomandazione lean keep-files) -- NON la vecchia ladder species-FIRST-then-biome/eco. Altro Mid: hardening audit-UI curator (AuditHistoryPanel + revert).
