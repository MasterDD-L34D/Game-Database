...
export default function ExportMenu<T>({ filename, rows, fields, serverQuery }: { filename: string; rows: T[]; fields: Field<T>[]; serverQuery?: string; }) {
  ...
  async function serverDownload(fmt: 'csv'|'json') {
    ...
    } catch (e: any) { alert(`Errore export: ${e.message || e}`); } finally { setBusy(false); }
  }

  return (<>
    <Button variant="outlined" onClick={(e)=>setAnchorEl(e.currentTarget)} startIcon={busy ? <CircularProgress size={16}/> : <ArrowDownTrayIcon className="h-5 w-5" />}>Esporta</Button>
    <Menu anchorEl={anchorEl} open={open} onClose={()=>setAnchorEl(null)}>
      {serverQuery && (<>
        <MenuItem onClick={()=>{ setAnchorEl(null); serverDownload('csv'); }}><ListItemText>CSV (server)</ListItemText></MenuItem>
        <MenuItem onClick={()=>{ setAnchorEl(null); serverDownload('json'); }}><ListItemText>JSON (server)</ListItemText></MenuItem>
      </>)}
      <MenuItem onClick={()=>{ setAnchorEl(null); downloadText(`${filename}.csv`, 'text/csv;charset=utf-8', toCsv()); }}><ListItemText>CSV (client)</ListItemText></MenuItem>
      <MenuItem onClick={()=>{ setAnchorEl(null); downloadText(`${filename}.json`, 'application/json;charset=utf-8', JSON.stringify(rows, null, 2)); }}><ListItemText>JSON (client)</ListItemText></MenuItem>
    </Menu>
  </>);
}
